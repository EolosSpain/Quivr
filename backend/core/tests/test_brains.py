...
from backend.core.models.brains import Brain, get_default_user_brain
...
def test_set_as_default_brain_endpoint(client, api_key):
    ...
    response = client.post(
        ...
        headers={"Authorization": "Bearer YOUR_API_KEY"},  # Replace YOUR_API_KEY with the actual API key of the user
    )
    ...
    assert response.json() == {"message": f"Brain {brain_id} has been set as default brain."}
